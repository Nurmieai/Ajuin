            ...
            <div class="modal-box w-full max-w-md bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 shadow-xl rounded-xl p-0 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div class="p-8 space-y-6 overflow-y-auto">
                    ...
                    <div class="space-y-2">
                        ...
                        @php
                        // Memanggil perhitungan dari controller
                        $roundedCompanyRating = round($companyAverageRating);
                        @endphp
                        <div class="flex items-center justify-between w-full bg-slate-50/50 dark:bg-slate-900/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
                            <span class="text-xs font-semibold text-slate-500">Rata-rata Perusahaan</span>
                            <div class="flex items-center gap-1.5">
                                <div class="flex items-center gap-0.5">
                                    @for($i = 1; $i <= 5; $i++)
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-4 {{ $i <= $roundedCompanyRating ? 'text-yellow-400' : 'text-slate-300 dark:text-slate-700' }}">
                                        <path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clip-rule="evenodd" />
                                        </svg>
                                        @endfor
                                </div>
                                <span class="font-bold text-slate-700 dark:text-slate-200 text-sm">{{ number_format($companyAverageRating, 1) }}/5</span>
                            </div>
                        </div>
                    </div>
